const fromEntries =
	...
	function fromEntries(iterable: any) {
		let entries = Array.isArray(iterable)
 ...
 * @returns rows数据
 */
export function clipboardParser(
	event: ClipboardEvent,
	options: ClipboardParserOptionsType
) {
	...
	let text: string = clipboardData
		...
		isApiModel: boolean =
			text.includes('@ApiModelProperty') || text.includes('private '),
		...
		rows.replace(
			...
			(
				...
				params.push({
					...
					required:
						typeof param1.required !== 'undefined'
							? param1.required
							: true,
					name: param1.value || f,

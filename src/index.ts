 ...
 * @returns rows数据
 */
export function clipboardParser(event: ClipboardEvent, options: ClipboardParserOptionsType) {
    ...
    let text: string = clipboardData
            ...
            .replace(/\r/g, '\n')
            .replace(/[\n\r]+$/, ''),
        isRequestParam: boolean = text.indexOf('@RequestParam') > -1,
        isApiModel: boolean = text.indexOf('@ApiModelProperty') > -1 || text.indexOf('private ') > -1,
        notTableData: boolean = text.indexOf('\t') === -1 || isRequestParam,
        reg: RegExp,
        len: number,
        rows: any = text.split('\n'),
        params: any[] = []
